...
public class OAuth2Controller {
    ...
    private String onLoginSuccess(User user, HttpSession session) {
        ...
        return "redirect:" + frontBaseUrl + "/oauth/success";
    }
    ...
    public String kakaoCallback(@RequestParam String code, HttpSession session) {
        try {
            ...
            if (nickname == null || nickname.isBlank()) nickname = "카카오 로그인 유저";
    ...
    public String googleCallback(@RequestParam String code, HttpSession session) {
        try {
            ...
            if (nickname == null || nickname.isBlank()) nickname = "구글 로그인 유저";

            User user = authService.findBySocialId(socialId, 3);

            if (user == null) {
                User newMember = new User();
                newMember.setEmail(email);
                newMember.setNickName(nickname);
                newMember.setLoginType(3);
                newMember.setSocialId(socialId);
                newMember.setLoginPw(null);
                newMember.setProfileImg(profileImg);

                authService.joinSocial(newMember);
                user = authService.findBySocialId(socialId, 3);
            }
